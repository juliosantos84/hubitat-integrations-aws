# hubitat-integrations-aws

This project contains source code and supporting files for a serverless application that you can deploy with the SAM CLI. It includes the following files and folders.

- main - Code for the application's Lambda function and Project Dockerfile.
...
```bash
door-notification-handler$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
door-notification-handler$ sam logs -n HelloWorldFunction --stack-name door-notification-handler --tail
```
...
```bash
door-notification-handler$ pip install pytest pytest-mock --user
door-notification-handler$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name door-notification-handler
```
